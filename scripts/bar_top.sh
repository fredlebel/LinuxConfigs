...
while true; do
  if [ "$(hostname)" = turing ]; then
    ...
    net_=$(Net_ wlp2s0)
    sleep 1;
    echo "%{l}$(ResetBar) $(Bspwm)%{r}$(Cpu $cpu_) $sep $(Mem) $sep $(Swap) $sep $(Disk /dev/mapper/MyStorage-rootvol Disk) $sep $(Ssid) $(Net $net_ wlp2s0) $sep $(Temp) $sep $(Brightness) $sep $(Power) $sep $(Battery) $sep $(Date) "
  fi
done
